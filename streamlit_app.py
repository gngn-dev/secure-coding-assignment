...
def main():
    ...
    if st.session_state.logged_in:
        if st.session_state.user["role"] == 'admin':
            ...
            menu = ['Home', 'Add Product', 'View Order', 'Edit Order - destination', 'Edit Order - state']
            ...
            # NEW
                ...
                with st.form(key='view_order_form'):
                    order_num = st.text_input('Order Number')
                    get_order_button = st.form_submit_button(label='view order')
                    
                    if get_order_button:
                        response = requests.get('http://localhost:8000/orders_by_num', params={"order_num": int(order_num)})
                        order = response.json()
                        if order != None:
                            st.write(f"Order Number: {order[0]}, Username: {order[1]}, Items: {order[2]}, Destination: {order[3]}, Date: {order[4]}, payment: {order[5]}, payment_data: {order[6]}, state: {order[7]}")
                        else:
                            st.error("Cannot Found. Order Number : " + order_num)

            # NEW
            elif choice == 'Edit Order - destination':
                st.subheader('Edit Order - destination')
                with st.form(key='edit_order_form'):
                    order_num = st.text_input('Order Number')
                    dest_address = st.text_input('destination address')
                    edit_order_dest_button = st.form_submit_button(label='update destination')
                    
                    if edit_order_dest_button:
                        response = requests.get('http://localhost:8000/update_orders', params={"order_num": int(order_num), "value": dest_address, "column_name": "dest_address"})
                        if response.status_code == 200:
                            ...
                            st.error("Failed to update order-destination information.")

            # NEW
            elif choice == 'Edit Order - state':
                st.subheader('Edit Order - state')
                with st.form(key='edit_order_form'):
                    order_num = st.text_input('Order Number')
                    state = st.text_input('state')
                    edit_order_state_button = st.form_submit_button(label='update state')

                    if edit_order_state_button:
                        response = requests.get('http://localhost:8000/update_orders', params={"order_num": int(order_num), "value": state, "column_name": "state"})
                        if response.status_code == 200:
                            if response.json()["status"] == "success":
                                st.success(response.json()["message"])
                            else:
                            ...
                            st.error("Failed to update order-state information.")

        else:
            st.sidebar.subheader('User Menu')
            menu = ['Home', 'Buy Products', 'My Page', 'Order List']
            choice = st.sidebar.selectbox('Menu', menu)
            ...
            if choice == 'Home':
                ...
                if st.button('Buy'):
                    ...
                    response = requests.get('http://localhost:8000/add_order', params={"username": st.session_state.user["username"], "selected_product": selected_product, "dest_address": user_address, "DATE": time.time(), "pay_PG": "test", "pay_PG_DATA": "test_PG_DATA", "status": "not checked"}) # for test
            ...
            # NEW
            elif choice == 'Order List':
                st.subheader('Order List')
                response = requests.get('http://localhost:8000/orders_by_username', params={"username": st.session_state.user["username"]})
                orders = response.json()
                if orders:
                    for order in orders:
                        st.write(f"Order Number: {order['order_num']}, Items: {order['selected_product']}, Destination: {order['dest_address']}, Date: {order['DATE']}, payment: {order['pay_PG']}")
                else:
                    st.write("Cannot found your order.")
